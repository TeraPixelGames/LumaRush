...
const moduleUnderTest = require("../modules/lumarush.js");
...
test("submit score applies OPEN mode tie-breaker fields", () => {
  moduleUnderTest.__setModuleConfigForTests({
    leaderboardIds: { open: "lr_open", pure: "lr_pure" },
    eventUrl: "",
  });
  ...
  assert.equal(result.leaderboardId, "lr_open");
...
test("get/list leaderboard route by mode", () => {
  ...
  assert.equal(mine.leaderboardId, "lr_pure");
  ...
  assert.equal(listed.leaderboardId, "lr_open");
