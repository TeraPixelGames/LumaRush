...
var _wallet: Dictionary = {
	...
	"shop": {
		...
		"themeRentals": {},
		"powerups": {"undo": 0, "prism": 0, "hint": 0},
	},
}
...
func logout() -> Dictionary:
	var had_linked_account := is_linked_account()
	...
	if not _connect_enabled:
		...
		return {"ok": true, "linked_cleared": had_linked_account, "authenticated": false}

	_set_online_state("Connecting...")
	var device_result: Dictionary = await _authenticate_device()
	if not device_result.get("ok", false):
		_set_online_state("Offline")
		auth_state_changed.emit(false, "")
		return {
			"ok": false,
			"linked_cleared": had_linked_account,
	...
	await refresh_my_high_score()
	await refresh_leaderboard(_leaderboard_limit)
	await refresh_entitlements(true)
	await refresh_wallet(true)
	return {"ok": true, "linked_cleared": had_linked_account, "authenticated": true}
...
func update_username(new_username: String) -> Dictionary:
	...
	if typeof(data) == TYPE_DICTIONARY:
		...
		track_client_event("account.username_update_succeeded", {
			...
			"username_length": str(row.get("username", "")).length(),
		}, true)
...
func _authenticate_custom(terapixel_user_id: String, create_if_missing: bool = false) -> Dictionary:
	...
	var create_value := "true" if create_if_missing else "false"
	var response: Dictionary = await _request_json(
		HTTPClient.METHOD_POST,
		"/v2/account/authenticate/custom?create=%s" % create_value,
...
func _handle_magic_link_completion(data: Dictionary) -> void:
	...
	if not linked_profile_id.is_empty():
		SaveStore.set_terapixel_identity(
			linked_profile_id,
...
func _read_runtime_settings() -> void:
	...
	_client_events_enabled = _env_or_setting_bool(
		"LUMARUSH_CLIENT_EVENTS_ENABLED",
		"lumarush/client_events_enabled",
		DEFAULT_CLIENT_EVENTS_ENABLED
	)
	_client_event_sample_rate = _env_or_setting_float(
		"LUMARUSH_CLIENT_EVENT_SAMPLE_RATE",
		"lumarush/client_event_sample_rate",
...
func _resolve_export_target() -> String:
	var configured := _env_or_setting_string(
		"LUMARUSH_EXPORT_TARGET",
		"lumarush/export_target",
		""
	).to_lower()
	...
	if OS.has_feature("android"):
		return "android"
	var platform_name := str(_project_setting("lumarush/platform", "web")).to_lower()
