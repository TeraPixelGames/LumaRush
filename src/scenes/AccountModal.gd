...
func _on_send_magic_link_pressed() -> void:
	...
	var email := _sanitize_email_input(email_input.text)
	if email.is_empty():
...
func _on_redeem_merge_code_pressed() -> void:
	var code := _sanitize_merge_code_input(merge_code_input.text)
	if code.is_empty():
		status_label.text = "Enter a valid merge code."
		return
	merge_code_input.text = code
...
func _on_update_username_pressed() -> void:
	var desired := _sanitize_username_input(username_input.text)
	if desired.is_empty():
		status_label.text = "Username must be 3-20 chars: letters, numbers, _ or -."
		return
	username_input.text = desired
...
func _refresh_account_controls() -> void:
	...
	if linked:
		...
		if linked_email.is_empty():
			linked_email = _sanitize_email_input(email_input.text)
...
func _safe_email_domain(email: String) -> String:
	var cleaned := _sanitize_email_input(email)
	if cleaned.is_empty():
		return ""
...
func _sanitize_username_input(value: String) -> String:
