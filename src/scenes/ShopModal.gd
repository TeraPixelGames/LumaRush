...
const COIN_PACKS := [
	...
	{"product_id": "coins_3000_lumarush", "label": "3000 - $4.99", "price_usd": 4.99},
	{"product_id": "coins_7500_lumarush", "label": "7500 - $9.99", "price_usd": 9.99},
	{"product_id": "coins_20000_lumarush", "label": "20000 - $19.99", "price_usd": 19.99},
]
...
func _start_paypal_checkout(row: Dictionary) -> Dictionary:
	...
	var checkout_id := "lr_%d_%d" % [Time.get_unix_time_from_system(), Time.get_ticks_usec() % 1000000]
	var payload := {
		"checkout_id": checkout_id,
		"client_id": client_id,
		"currency": _resolve_paypal_currency(),
		"product_id": str(row.get("product_id", "")).strip_edges().to_lower(),
		"label": str(row.get("label", "LumaRush Coins")).strip_edges(),
		"amount": "%.2f" % amount,
	}
	var start_js := (
		"(function(){if(!window.LumaRushPayPalBridge){return 'missing';}"
		+ "window.LumaRushPayPalBridge.start(%s);return 'ok';})();"
...
func _take_paypal_checkout_result(checkout_id: String) -> Dictionary:
	if checkout_id.is_empty():
		return {}
	var js := (
		"(function(){window.__lumaRushPayPalResults=window.__lumaRushPayPalResults||{};"
		+ "var key=%s; if(!(key in window.__lumaRushPayPalResults)){return '';}"
		+ "var out=window.__lumaRushPayPalResults[key]; delete window.__lumaRushPayPalResults[key];"
...
func _ensure_paypal_bridge() -> void:
	...
	var bridge_js := """
	(function() {
	  if (window.LumaRushPayPalBridge) { return; }
	  window.__lumaRushPayPalResults = window.__lumaRushPayPalResults || {};
	  window.LumaRushPayPalBridge = {
	    sdkPromise: null,
		sdkKey: "",
	    ...
	    ensureOverlay: function() {
	      ...
	      close.onclick = function() {
	        ...
	        if (overlay._checkoutId) {
	          window.__lumaRushPayPalResults[overlay._checkoutId] = {
	            ok: false,
	            cancelled: true,
	    ...
	    start: function(opts) {
		  ...
		  var label = String(opts && opts.label || "LumaRush Coins");
		  var currency = String(opts && opts.currency || "USD").toUpperCase();
		  var amount = String(opts && opts.amount || "0.99");
	      if (!checkoutId) { return; }
	      if (!clientId) {
			window.__lumaRushPayPalResults[checkoutId] = { ok: false, error: "missing_client_id" };
	        return;
	      }
	      self.ensureSdk(clientId, currency).then(function() {
	        if (!window.paypal || !window.paypal.Buttons) {
			  window.__lumaRushPayPalResults[checkoutId] = { ok: false, error: "paypal_sdk_unavailable" };
	          return;
	        }
	        ...
	        if (!host) {
			  window.__lumaRushPayPalResults[checkoutId] = { ok: false, error: "paypal_host_missing" };
			  overlay.style.display = "none";
	          return;
	        }
			host.innerHTML = "";
	        window.paypal.Buttons({
	          ...
	          onApprove: function(data, actions) {
	            return actions.order.capture().then(function(details) {
	              ...
	              window.__lumaRushPayPalResults[checkoutId] = {
	                ok: true,
...
func _resolve_paypal_client_id() -> String:
	var from_env := OS.get_environment("LUMARUSH_PAYPAL_CLIENT_ID").strip_edges()
	if not from_env.is_empty():
		return from_env
	var from_setting := str(_project_setting("lumarush/paypal_client_id", _project_setting("color_crunch/paypal_client_id", ""))).strip_edges()
	if not from_setting.is_empty():
		return from_setting
	if _is_web_checkout_supported():
		var from_window: Variant = JavaScriptBridge.eval(
			"(function(){return String(window.LUMARUSH_PAYPAL_CLIENT_ID || window.COLOR_CRUNCH_PAYPAL_CLIENT_ID || window.TPX_PAYPAL_CLIENT_ID || '');})();",
			true
		)
...
func _resolve_paypal_currency() -> String:
	var currency := str(_project_setting("lumarush/paypal_currency", _project_setting("color_crunch/paypal_currency", "USD"))).strip_edges().to_upper()
...
func _on_buy_shuffle_pressed() -> void:
	await _on_buy_powerup_pressed("hint")
