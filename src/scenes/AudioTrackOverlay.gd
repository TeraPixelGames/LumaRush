...
@onready var track_selector: Control = $Center/Panel/Margin/VBox/TrackSelector
...
func setup(track_names: Array, selected_index: int) -> void:
	var normalized_tracks: Array[String] = []
	for track_name in track_names:
		normalized_tracks.append(str(track_name))
	track_selector.tracks = normalized_tracks
	if normalized_tracks.is_empty():
		track_selector.current_index = 0
	else:
		track_selector.current_index = clampi(selected_index, 0, normalized_tracks.size() - 1)
