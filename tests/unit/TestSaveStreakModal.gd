...
func test_modal_mouse_filters_capture_input() -> void:
	...
	var dim: Control = modal.get_node_or_null("Dim") as Control
	var panel: Control = modal.get_node_or_null("Panel") as Control
	var box: Control = modal.get_node_or_null("Panel/VBox") as Control
	assert_that(dim).is_not_null()
